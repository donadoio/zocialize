...
import { authLogout, authRefreshing, authSessionExpired, authUpdateTokens } from "./slices/authSlice";
            ...
            if (getState().auth && getState().auth.authenticated) {
                ...
                if (isExpired) {
                    if (!(getState().auth.refreshTokenPromise)) {
                        return refreshToken(getState().auth.refresh_token, dispatch).then(() => next(action));
                    } else {
...
export function refreshToken(refreshToken: string, dispatch) {
    ...
    var freshTokenPromise = axios.post(`http://localhost:3000/auth/refresh`, null, {
        headers: {
            Authorization: `Bearer ${refreshToken}`
        }
    }).then(t => {
        t.data ? dispatch(authUpdateTokens(t.data)) : dispatch(authSessionExpired());
        return t.data ? Promise.resolve(t.data) : Promise.reject({
